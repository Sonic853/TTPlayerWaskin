...
- guid:avs 复用 WSZ 视频窗口的歌词/视觉/同屏内容、原生菜单与编辑；Vis 跟随主窗口视觉模式，并按 WAL 绘制 Winamp 风格频谱/示波器。详见 [内容与 Vis 接入](WAL_CONTENT_VIS_2026_09_24.md)。不实现 Winamp AVS 引擎或其预设。
- 主窗口位置、HeadAMP 两侧抽屉和视觉切换状态持久化；独立播放列表、均衡器和歌词窗口继续使用内置默认皮肤及已有功能。
- WAL 背景拖动使用主程序的吸附、分离及同步窗口移动；捕获顺序和重入修复见 [WAL 拖动修复](WAL_DRAG_FIX_2026_09_24.md)。
